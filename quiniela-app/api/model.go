...
func (u *usuario) createUser(db *sql.DB) error {
	fmt.Println("nacimiento: ", u.Fecha_nacimiento)
	fmt.Println("registro: ", u.Fecha_registro)
	query := fmt.Sprintf(`INSERT INTO usuario VALUES(null, '%s', '%s', '%s', '%s', '%s', 0, TO_DATE('%s', 'dd/mm/yyyy'), TO_DATE('%s','dd/mm/yyyy'), utl_raw.cast_to_raw('%s'))`, u.Username, u.Correo, u.Nombre, u.Apellido, u.Contraseña, u.Fecha_nacimiento, u.Fecha_registro, u.Foto)
...
func (u *usuario) recoverUser(db *sql.DB) error {
	query := fmt.Sprintf(`SELECT contraseña FROM usuario WHERE username='%s'`, u.Username)
...
func getAllUsers(db *sql.DB, offset int, limit int) ([]usuario, error) {
	rows, err := db.Query(
		"SELECT username, correo, nombre, apellido FROM usuario")
	...
	for rows.Next() {
		var u usuario
		if err := rows.Scan(&u.Username, &u.Correo, &u.Nombre, &u.Apellido); err != nil {
			return nil, err
		}
